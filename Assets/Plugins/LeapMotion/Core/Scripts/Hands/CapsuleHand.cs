...
#pragma warning restore 0649
        ...
        public override void BeginHand() {
            base.BeginHand();
            // ! Do not change the color here, instead we load it separately from the material file
            // Buggy
            //string materialName = _handMaterialNames[_handMaterialNamesIndex];
            //_handMaterialNamesIndex = (_handMaterialNamesIndex + 1) % _handMaterialNames.Length;
            //Debug.Log(string.Format("Trying to load the material: {0}", materialName));
            //Material newMaterial = Resources.Load<Material>(materialName);
            // ! Loading from links
            Material newMaterial = _materials[_materialIndex];
            _materialIndex = (_materialIndex + 1) % _materials.Length;
            if (newMaterial != null) {
                // ? Should we copy the new material?
                _sphereMat = new Material(newMaterial);
                // Making it glow
                _sphereMat.SetColor("_EmissionColor", _sphereMat.color * 1.2f);
                _sphereMat.EnableKeyword("_EMISSION");
                //_sphereMat.SetTexture("_EmissionMap", _sphereMat.GetTexture("_MainTex"));

                Debug.Log("Good new material.");
            } else {
                ...
                Debug.LogWarning("Bad new material.");
            }
